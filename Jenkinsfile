...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t atemafacgeorge/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker atemafacgeorge/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push atemafacgeorge/frontendservice:latest "
                    }
                }
            }
        }
         ...
         stage('ZAP Dynamic Testing | DAST') {
             steps {
                 sshagent(['OWASP-Zap-Credential']) {
                     sh 'ssh -o StrictHostKeyChecking=no ubuntu@52.14.115.146 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://18.119.0.234:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#aat-multi-microservics-alerts', //update and provide your channel name
